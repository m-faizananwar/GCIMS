...
const DateRangePicker: React.FC<DateRangePickerProps> = ({ startDate, endDate }) => {
    ...
    const [selectedStartDate, setSelectedStartDate] = useState(startDate || undefined);
    const [selectedEndDate, setSelectedEndDate] = useState(endDate || undefined);
    const [isOpen, setIsOpen] = useState(false);

    const datepickerRef = useRef<HTMLDivElement>(null);
    
    // useEffect(() => {console.error(`start: ${selectedStartDate}, end: ${selectedEndDate}`)}, [selectedStartDate, selectedEndDate])

    const updateQuery = (start: string, end: string) => {        
    ...
    const renderCalendar = () => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            let className =
                "flex h-[46px] w-[46px] items-center justify-center rounded-full hover:cursor-pointer hover:bg-[#1C0F13] hover:text-white my-[0.5]";

            if (selectedStartDate && dayString === selectedStartDate) {
                className += " bg-[#1C0F13] text-white rounded-r-none";
            }
            if (selectedEndDate && dayString === selectedEndDate) {
                className += " bg-[#1C0F13] text-white rounded-l-none";
            }
            if (
                selectedStartDate &&
                selectedEndDate &&
                ...
                className += " bg-[#D8C5C8] rounded-none";
            }
    ...
    const handleDayClick = (selectedDay: any) => {
        if (!selectedStartDate || (selectedStartDate && selectedEndDate)) {
            updateQuery(selectedDay, "undefined")
            ...
            if (new Date(selectedDay) < new Date(selectedStartDate)) {
                ...
                setSelectedStartDate(selectedDay);
                updateQuery(selectedDay, selectedStartDate)
            } else {
                setSelectedEndDate(selectedDay);
                updateQuery(selectedStartDate, selectedDay)
            }
        }
    };
    ...
    return (
        <section className="w-full">
            <div className="w-full container select-">
                <div className="w-full" ref={datepickerRef}>
                    <div className="relative">
                        <input
                            type="text"
                            placeholder="Select dates..."
                            className="h-12 w-full rounded-full outline outline-2 outline-slate-200 hover:outline-slate-400 bg-white pl-12 pr-4 text-dark focus:outline-slate-400 hover:cursor-pointer"
                            value={updateInput()}
                    ...
                    {isOpen && (
                        <div className="absolute mt-3 shadow-lg flex w-auto flex-col rounded-3xl outline outline-2 outline-slate-400 bg-white px-4 py-6">
                            <div className="flex items-center justify-between pb-2">
                                ...
                                <div className="flex items-center justify-end space-x-[10px]">
                                    <span
                                        ...
                                        className="flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded border-[.5px] border-stroke bg-white text-[#1C0F13] hover:bg-[#1C0F13] hover:text-white"
                                    >
                                        <svg
                                            width="16"
                                            height="16"
                            ...
                            <div className="grid grid-cols-7 pb-2 pt-4 text-sm font-normal capitalize text-gray-500">
                            ...
                            <div className="flex items-center justify-between pt-4">
                                <button className="h-[37px] rounded-full border border-stroke bg-[#1C0F13] px-5 text-sm font-medium text-white hover:cursor-default">
                                    {selectedStartDate}
                                </button>
                                -
                                <button className="h-[37px] rounded-full border border-stroke bg-[#1C0F13] px-5 text-sm font-medium text-white hover:cursor-default">
